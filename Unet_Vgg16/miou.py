...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):  
    ...
    gt_imgs = [join(gt_dir, x + ".JPG") for x in png_name_list]  # 获得验证集标签路径列表，方便直接读取
    pred_imgs = [join(pred_dir, x + ".jpg") for x in png_name_list]  # 获得验证集图像分割结果路径列表，方便直接读取
...
if __name__ == "__main__":
    gt_dir = "H:/unet-keras-master/VOCdevkit/VOC2007/SegmentationClass/"
    pred_dir = "H:/unet-keras-master/img/pre/"
    png_name_list = open(r"H:/unet-keras-master/VOCdevkit/VOC2007/ImageSets/Segmentation/test.txt",'r').read().splitlines() 
    
    num_classes = 2
    name_classes = ["background","tassel"]
